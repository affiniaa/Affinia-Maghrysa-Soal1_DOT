...
class Transaksi_model extends CI_Model {
	...
	{
		return $this->db->where('cart_id',$this->session->userdata('username'))
	...
	{
		return $this->db->select('transaksi.id_transaksi, transaksi.nama_pembeli, transaksi.id_kasir, transaksi.tanggal_beli, (SELECT SUM(detail_transaksi.jumlah*buku.harga) FROM detail_transaksi JOIN buku ON buku.id_buku = detail_transaksi.id_buku WHERE id_transaksi = transaksi.id_transaksi ) as total')
						->join('detail_transaksi','detail_transaksi.id_transaksi = transaksi.id_transaksi')
						->join('buku','buku.id_buku = detail_transaksi.id_buku')
	...
	{
		return $this->db->select('buku.judul_buku, kategori.nama_kategori, buku.penulis, buku.penerbit, buku.tahun, buku.harga, detail_transaksi.jumlah')
